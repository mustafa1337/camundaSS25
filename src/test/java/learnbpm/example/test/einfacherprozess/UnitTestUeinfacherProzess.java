...
public class UnitTestUeinfacherProzess {
	...
	@Test
	@Deployment(resources = "Immatrikulation_SOLL_CAMUNDA7.bpmn") // hier die Prozessmodelle aufführen die getest werden sollen
	public void testProzess() {

		// Prozessinstanz des Prozessmodells mit dem Key (=Id des Prozessmodells)
		// erzeugen und starten

		ProcessInstance pi = processEngine().getRuntimeService().startProcessInstanceByKey("ImmatrikulationSOLL");
		System.out.println("Prozessinstanz mit der Id "+ pi.getId()+ " gestartet");
	}

	// JUnit-Test für einen Prozessteil
	@Test
	@Deployment(resources = "Immatrikulation_SOLL_CAMUNDA7.bpmn")
	public void testTeilprozess() {

		// Prozessinstanz starten vor der Aktivität mit dem Key (=Id des Elements im
		// Prozessmodell)
		//ProcessInstance pi = processEngine().getRuntimeService().createProcessInstanceByKey("ImmatrikulationSOLL");
				//.startBeforeActivity("gateway_unterlagen_vollstaendig")
				//.setVariable("unterlagen_vollstaendig", true) // Prozessvariable erzeugen und mit Wert belegen
				//.execute();
		ProcessInstance pi = processEngine().getRuntimeService()
                .startProcessInstanceByKey("ImmatrikulationSOLL");
