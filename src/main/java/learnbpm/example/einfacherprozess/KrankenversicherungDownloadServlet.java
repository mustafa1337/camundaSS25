...
@WebServlet("/download/kv")
public class KrankenversicherungDownloadServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String sql = "SELECT krankenversicherung FROM immatrikulationsantrag WHERE id = ?";
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
            ...
            if (rs.next()) {
                byte[] data = rs.getBytes("krankenversicherung");

                resp.setContentType("application/pdf");
                resp.setHeader("Content-Disposition", "attachment; filename=\"Krankenversicherung.pdf\"");
